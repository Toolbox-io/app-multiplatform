...
actual fun String.encrypt(password: String): String {
    ...
    val paddedData = padData(encodeToByteArray())
    ...
    for (i in paddedData.indices step 16) {
        ...
        paddedData.copyInto(block, 0, i, i + 16)
...
actual fun String.decrypt(password: String): String {
    ...
    for (i in encryptedBytes.indices step 16) {
        val block = ByteArray(16)
        encryptedBytes.copyInto(block, 0, i, i + 16)
    ...
    return unpaddedData.contentToString()
}
...
private fun String.decodeBase64(): ByteArray {
    ...
    memScoped {
        val data = NSData.create(this@decodeBase64)!!
        val bytes = data.bytes ?: return ByteArray(0)
        result = bytes.readBytes(data.length.toInt())
    }
    return result
}
